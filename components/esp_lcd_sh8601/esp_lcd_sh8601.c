                                   ...
                                   esp_lcd_panel_handle_t *ret_panel)
{
    ...
    sh8601_panel_t *sh8601 = calloc(1, sizeof(sh8601_panel_t));
    ...
    case 16:
        ...
        sh8601->fb_bits_per_pixel = 16;
        break;
    case 18:
        sh8601->colmod_val = 0x66;
        sh8601->fb_bits_per_pixel = 24;
        break;
    case 24:
    ...
    *ret_panel = &(sh8601->base);
    ESP_LOGI(TAG, "new sh8601 panel @%p, version: %d.%d.%d", sh8601,
             ESP_LCD_SH8601_VER_MAJOR, ESP_LCD_SH8601_VER_MINOR, ESP_LCD_SH8601_VER_PATCH);
    return ESP_OK;
...
static esp_err_t tx_param(sh8601_panel_t *sh8601, esp_lcd_panel_io_handle_t io,
...
static esp_err_t tx_color(sh8601_panel_t *sh8601, esp_lcd_panel_io_handle_t io,
                          int lcd_cmd, const void *param, size_t param_size)
{
    if (sh8601->flags.use_qspi_interface) {
        lcd_cmd &= 0xff;
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t *panel)
{
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_MADCTL,
                                 (uint8_t[]){sh8601->madctl_val}, 1), TAG, "send command failed");
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_COLMOD,
                                 (uint8_t[]){sh8601->colmod_val}, 1), TAG, "send command failed");
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ESP_RETURN_ON_ERROR(tx_param(sh8601, io, init_cmds[i].cmd,
...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t *panel,
                                          ...
                                          const void *color_data)
{
    ...
    ESP_RETURN_ON_ERROR(tx_color(sh8601, io, LCD_CMD_RAMWR, color_data, len),
                        TAG, "send color data failed");
    return ESP_OK;
}
...
static esp_err_t panel_sh8601_invert_color(esp_lcd_panel_t *panel, bool invert_color_data)
{
    ...
    return tx_param(sh8601, io, cmd, NULL, 0);
}
...
static esp_err_t panel_sh8601_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y)
{
    ...
    if (mirror_y) {
        ESP_LOGE(TAG, "mirror_y is not supported");
        return ESP_ERR_NOT_SUPPORTED;
    }
    return tx_param(sh8601, io, LCD_CMD_MADCTL, (uint8_t[]){sh8601->madctl_val}, 1);
}

static esp_err_t panel_sh8601_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
{
    ESP_LOGE(TAG, "swap_xy is not supported");
    return ESP_ERR_NOT_SUPPORTED;
}
